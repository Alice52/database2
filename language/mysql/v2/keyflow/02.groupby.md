...
3.  使用临时表实现 group by
    ...
    - 执行流程
      ...
      3. 遍历完成后, 再根据字段 m 做排序{如果 sort_buffer 不够则会使用文件排序}, 得到结果集返回给客户端
      4. 过程中如果内存临时表满了{tmp_table_size:16M}, 就会把内存临时表转成磁盘临时表{磁盘临时表就会变的有序的(innodb 的 b+tree)}
...
2. 去掉临时表{加索引}: 使用紧凑索引扫描或者松散索引扫面

   - 直接加索引
   - generated column 机制: 设计计算的

3. [不适合间索引+临时表会放很多数据]直接排序: `直接使用排序算法后计算groupby结果`
...
4. 5.7 及之前不需要对结果集排序则写明 `order by null`
